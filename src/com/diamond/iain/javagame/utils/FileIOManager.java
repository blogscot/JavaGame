...
public class FileIOManager {
	...
	/**
	 * The very first time the game is played the high score file won't
	 * exist so the exception will be raised. If the error message is
	 * displayed after that there is a 'real' problem.
	 * 
	 * @param path 	Of the high score file
	 * @return		the stored high score
	 */
	public static int readHighScoreFromFile() {

		// Uses try-with-resources to close Buffered Reader
		try (BufferedReader br = new BufferedReader(new FileReader(HighScoresPath))) {
	...
	public static void writeHighScoreToFile(Integer score) {
		...
		try (BufferedWriter bw = new BufferedWriter(new FileWriter(HighScoresPath))) {
